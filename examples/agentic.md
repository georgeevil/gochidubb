...
Once the MCP server is wired up in your agent (`claude mcp add gochidubb …`),
just talk to the agent. These are prompts that work reliably with the
bundled `.claude/skills/gochidubb/SKILL.md`.
...
The agent calls `gochidubb_dub(source=..., target_lang="fr", wait=False)`
...
The agent calls `gochidubb_dub(..., wait=True)` and reports the URL.
...
The agent calls `gochidubb_showcase(source=..., target_langs=[...],
...
The agent calls `gochidubb_compare(...)` and returns the URLs in a table.

## Re-dub from history

> The French one I made yesterday — also do Japanese and Italian.

The agent calls `gochidubb_list_jobs(status="complete")`, finds the
matching job, and calls `gochidubb_redub(job_id, target_langs=["ja","it"],
mode="compare")`. No re-upload, no re-transcribe.

## Pipeline health check

> What's the state of my GoChiDUBB setup? Anything missing?

The agent calls `gochidubb_system_status()` and surfaces missing
...
The agent loops `gochidubb_list_jobs(limit=5)` → `gochidubb_redub(...)`
...
The agent calls `gochidubb_rebuild_showcase("sc_31623e75")` — re-runs
